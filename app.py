...
def register():
    if request.method == 'POST':
        ...
        if not re.match(r"^\w{3,50}$", u):
            flash("Username must be 3-50 alphanumeric chars.", "danger")
...
def login():
    if request.method == 'POST':
        ...
        if se.is_ip_blacklisted(cur, ip):
            ...
            conn.commit(); cur.close(); conn.close()
            flash("Your IP is blacklisted due to suspicious activity.", "danger")
            return redirect(url_for('login'))
        ...
        if row and check_password_hash(row[1], p):
            ...
            if row:
                se.register_failed_attempt(cur, row[0])
...
def dashboard():
    ...
    cur.execute("SELECT COUNT(*) FROM login_logs WHERE success=1")
    success_logins = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM login_logs WHERE success=0")
    failed_logins = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM ip_blacklist")
    ...
    cur.execute("""SELECT DATE(attempt_time),
                          SUM(success=1), SUM(success=0)
                   FROM login_logs
                   WHERE attempt_time >= NOW() - INTERVAL 7 DAY
                   GROUP BY DATE(attempt_time) ORDER BY 1""")
    timeline = cur.fetchall()

    cur.execute("""SELECT ip, COUNT(*) FROM login_logs
                   WHERE success=0 GROUP BY ip
    ...
    return render_template('dashboard.html',
        total_users=total_users, success_logins=success_logins,
        failed_logins=failed_logins, blacklisted=blacklisted,
        alerts_24h=alerts_24h, timeline=timeline, top_ips=top_ips,
...
def api_live_stats():
    ...
    cur.close(); conn.close()
    return jsonify({"failed": fails, "success": succ, "alerts_1h": alerts})
...
def logs_view():
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("""SELECT id,username_attempted,ip,user_agent,
...
def logs_export():
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(['ID', 'Username Attempted', 'IP', 'User Agent', 'Success', 'Time'])
    w.writerows(rows)
...
def blacklist_view():
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("""SELECT id,ip,reason,auto_added,added_at
                   FROM ip_blacklist ORDER BY added_at DESC""")
    rows = cur.fetchall(); cur.close(); conn.close()
...
def blacklist_add():
    ...
    if not re.match(r"^\d{1,3}(\.\d{1,3}){3}$", ip):
        flash("Invalid IP format.", "danger")
        return redirect(url_for('blacklist_view'))
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute(
        """INSERT IGNORE INTO ip_blacklist(ip,reason,auto_added)
           VALUES(%s,%s,0)""",
        (ip, request.form.get('reason', 'Manual block'))
    )
    conn.commit(); cur.close(); conn.close()
    flash("IP added to blacklist.", "success")
    return redirect(url_for('blacklist_view'))
...
def blacklist_remove(bid):
    ...
    conn.commit(); cur.close(); conn.close()
    flash("Removed from blacklist.", "info")
    return redirect(url_for('blacklist_view'))
