...
def create_alert(cur, alert_type, message, severity, ip=None):
    cur.execute(
        """INSERT INTO alerts(alert_type,message,severity,related_ip)
           VALUES(%s,%s,%s,%s)""",
...
def log_login_attempt(cur, user_id, username, ip, user_agent, success):
    cur.execute(
        """INSERT INTO login_logs
           (user_id,username_attempted,ip,user_agent,success)
           VALUES(%s,%s,%s,%s,%s)""",
...
def reset_failed_attempts(cur, user_id, ip):
    cur.execute(
        """UPDATE users SET failed_attempts=0, locked_until=NULL,
           last_login=NOW(), last_ip=%s WHERE id=%s""",
        (ip, user_id)
    )
...
def detect_brute_force(cur, ip):
    ...
    cur.execute(
        """SELECT COUNT(*) FROM login_logs
           WHERE ip=%s AND success=0 AND attempt_time >= %s""",
        (ip, since)
    )
    ...
    if recent_fails >= BRUTE_FORCE_THRESHOLD:
        create_alert(
            cur, "BRUTE_FORCE",
            f"Brute-force from {ip}: {recent_fails} fails in "
            f"{BRUTE_FORCE_WINDOW_MIN} min",
            "Critical", ip
        )

    cur.execute("SELECT COUNT(*) FROM login_logs WHERE ip=%s AND success=0", (ip,))
    total_fails = cur.fetchone()[0]
    if total_fails >= AUTO_BLACKLIST_THRESHOLD:
        auto_blacklist_ip(cur, ip, f"Auto: {total_fails} failed login attempts")


def detect_credential_stuffing(cur):
    since = datetime.now() - timedelta(minutes=BRUTE_FORCE_WINDOW_MIN)
    cur.execute(
        """SELECT ip, COUNT(DISTINCT username_attempted) AS uniq_users
           FROM login_logs
           WHERE success=0 AND attempt_time >= %s
           GROUP BY ip HAVING uniq_users >= %s""",
        (since, CRED_STUFFING_USERS)
    )
    for ip, n in cur.fetchall():
        create_alert(
            cur, "CREDENTIAL_STUFFING",
            f"IP {ip} tried {n} different usernames",
            "Critical", ip
        )
